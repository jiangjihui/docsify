...
> **HashMap 底层结构**：底层为数组（桶），每个桶挂链表或红黑树，链表长度 ≥8 且桶数 ≥64 时树化。

```mermaid
flowchart LR
    subgraph TABLE["table 数组（默认容量 16）"]
        B0["桶[0]"]
        B1["桶[1]"]
        BK["桶[k]"]
    end
    B0 --> L0["链表 Node<br/>key / value / hash / next"]
    B1 --> R1["红黑树 TreeNode"]
    BK --> LK["链表 Node"]
    NOTE["阈值：链表长度≥8 且 桶数≥64 → 树化"]
```
...
> **HashMap put 流程**：定位桶 → 空则直接放；红黑树则树插入；链表则尾插，达到阈值触发树化；存在相同 key 则覆盖。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 10}}}%%
flowchart TD
    P["put(key, value)"] --> H["计算 hash 定位桶 i"]
    H --> E{"桶 i 为空?"}
    E -->|"是"| N["直接放 Node"]
    E -->|"否"| T{"节点是 TreeNode?"}
    T -->|"是"| TP["红黑树插入"]
    ...
    L -->|"是"| OV["覆盖原 value"]
    L -->|"否"| LT["尾插到链表末尾"]
    LT -->|"链表长度≥8"| TR["treeifyBin 树化（桶数≥64）"]
    N --> INC["size++ 超阈值则 resize"]
    OV --> RET["返回旧值"]
    TP --> INC
    TR --> INC
```
...
> **ConcurrentHashMap 实现演进**：JDK7 用分段锁（Segment）降低锁粒度，JDK8 改为数组+链表+红黑树并用 CAS + synchronized 加锁。

```mermaid
flowchart LR
    subgraph J7["JDK 1.7：分段锁"]
        SEG["Segment 数组<br/>（默认 16 段）"]
        SEG --> S0["Segment 0<br/>类似 Hashtable"]
        SEG --> S1["Segment 1"]
        SEG --> SN["Segment N"]
    end
    subgraph J8["JDK 1.8：CAS + synchronized"]
        T8["table 数组 + 链表/红黑树"]
        T8 -->|"桶头 CAS"| CAS["CAS 抢桶"]
        T8 -->|"链表/树节点"| SY["synchronized 加锁"]
    end
```
