...
- **G1收集器** 优先回收垃圾最多的Region区域，可指定垃圾收集的停顿时间（默认为200毫秒），可以不用一次将所有垃圾收集完。（保证现有空间够用即可）
...
- **CMS**
  分代收集：分为新生代和老年代，新生代和老年代是物理隔离的。对于新生代，采用复制算法；对于老年代，采用标记 - 压缩算法。

- **G1**
  分区收集：将整个 Java 堆划分为多个大小相等的独立区域（Region），这些 Region 可以是 Eden 区、Survivor 区或者老年代等不同类型。这种布局打破了传统的年轻代和老年代的物理隔离，使得 G1 可以更加灵活地进行垃圾回收操作。
...
主要用于查看和修改正在运行的 Java 进程的系统属性。可以查看指定 Java 进程的系统属性和 JVM 参数值。例如，可以查看堆大小设置、垃圾收集器类型、是否启用压缩指针等参数。这对于了解正在运行的应用程序的配置非常有用。
...
#### 一、使用`top`命令查找高CPU占用进程

1. **执行`top`命令**：这将显示当前系统中所有进程的CPU和内存使用情况，默认情况下按CPU使用率排序。
   - 重点关注`PID`（进程ID）和`%CPU`（CPU使用率）列。
2. **确定高CPU占用进程**：找到CPU使用率最高的进程，通常这个进程就是导致CPU飙升的元凶。

#### 二、深入分析进程中的线程

1. **使用`top -Hp PID`命令**：针对上一步找到的进程ID（PID），执行此命令查看该进程下所有线程的CPU使用情况。
   - 这将列出该进程内的所有线程及其CPU使用率。
2. **确定高CPU占用线程**：找到CPU使用率最高的线程，并记录其线程ID（通常是LWP或NID列的值）。

#### 三、将线程ID转换为十六进制

由于`jstack`等Java工具在输出线程堆栈时通常使用十六进制格式，因此需要将线程ID转换为十六进制。

- 使用命令`printf "%x\n" <线程ID>`进行转换。

#### 四、使用`jstack`命令查看线程堆栈

1. **执行`jstack PID | grep <线程ID十六进制值>`**：将`jstack`命令的输出通过`grep`命令过滤，找到与前面转换得到的十六进制线程ID相匹配的线程堆栈信息。
   - 注意：如果线程堆栈信息很长，可能需要调整`grep`命令的输出范围，如使用`-A 30`参数来打印匹配行之后的30行。
2. **分析线程堆栈**：查看线程的堆栈信息，找出导致CPU飙升的具体代码位置。
   - 当线程名称是VM Thread 的话，那就是GC线程，大概率是GC频繁导致的CPU飙升。
   - 注意查看是否有死锁（deadlock）、等待（WAITING）、阻塞（BLOCKED）等状态，以及是否有耗时的操作或无限循环等。

[**排查示例**](https://blog.csdn.net/blade2001/article/details/9065985)
