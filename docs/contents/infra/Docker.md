...
> **Docker 客户端-服务器架构**：Client 通过 REST API 与 Daemon 通信，Daemon 管理容器与运行时。
...
> **镜像分层结构**：镜像由多层只读层叠加组成，每层可被复用，容器在顶层叠加可写层。

```mermaid
flowchart TB
    subgraph IMG["镜像（只读，分层存储，层可复用）"]
        L0["基础层 Base Image"]
        L1["层 1：安装依赖"]
        L2["层 2：拷贝应用"]
        L3["层 3：启动命令"]
    end
    CW["容器可写层（运行时修改，删除容器即丢失）"]
    L0 --> L1 --> L2 --> L3 --> CW
```
