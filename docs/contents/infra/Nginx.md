...
> **Master-Worker 进程模型**：Master 管理进程 fork 出多个平等的 Worker，由 accept_mutex 竞争处理请求。
...
> **反向代理与负载均衡拓扑**：Nginx 作为反向代理，按策略将请求分发到 upstream 中的多个后端服务器。

```mermaid
flowchart LR
    C["客户端"] --> NG["Nginx 反向代理 + 负载均衡"]
    subgraph UP["upstream backend"]
        B1["后端 server 127.0.0.1:8080"]
        B2["后端 server 127.0.0.1:8081"]
    end
    NG -->|"轮询 / 权重 / ip_hash / least_conn"| B1
    NG -->|"负载均衡策略分发"| B2
```
...
> **正向代理与反向代理对比**：正向代理代理「客户端」访问外部，反向代理代理「服务端」、对客户端隐藏真实服务器。

```mermaid
flowchart LR
    subgraph F["正向代理"]
        C1["客户端"] --> PF["代理服务器"] --> SF["外部资源服务器"]
    end
    subgraph R["反向代理"]
        C2["客户端"] --> PR["代理服务器"] --> SR["内部真实服务器（客户端不可见）"]
    end
```
