...
- **保存消费记录**
  ...
  比如使用redis记录消息正在消费，消费完成则更新状态为消费结束。



### RabbitMQ吞吐量为何不及RocketMQ

**存储机制**

- 当消息需要持久化时（即需要在重启后仍然可用），RabbitMQ 将确保消息在发送给消费者之前先写入磁盘。这一步骤增加了延迟，但也增加了可靠性。（没有异步刷盘）

**负载均衡**：

- **RocketMQ**：采用消息路由机制，生产者可以将消息均匀分布到多个 Broker 上，消费者可以从任意 Broker 拉取数据。
- **RabbitMQ**：依赖于队列复制机制来实现负载均衡，可能导致额外延迟和管理复杂度。
...
（Exchanges）：交换机一方面：接收生产者发送的消息。另一方面：知道如何处理消息，例如递交给某个特别队列、递交给所有队列、或是将消息丢弃。决定消息如何投递，取决于Exchange的类型。

Exchange类型有以下几种：

- **Direct**：直连，通过 routing key 路由键将交换机和队列进行绑定，只将消息发送给完全匹配此 routing key 的队列。

- **Fanout**：广播，将消息交给所有绑定到交换机的队列（routing key无效）

- **Topic**：通配符，与direct类似，也是需要通过routing key路由键进行匹配和分发，区别在于Topic可以进行模糊匹配，Direct是完全匹配。
