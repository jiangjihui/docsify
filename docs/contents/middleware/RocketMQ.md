...
> **RocketMQ 整体架构**：NameServer 提供路由发现，Broker 负责存储转发，生产者与消费者围绕其工作。

```mermaid
flowchart TB
    NS[("NameServer<br/>命名服务 · 无状态<br/>Broker 注册 / 路由发现")]
    subgraph B["Broker 集群"]
        BM["Master Broker<br/>接收 · 存储 · 转发"]
        BS["Slave Broker<br/>主从复制"]
    end
    BM <-->|"主从复制"| BS
    P["Producer<br/>发送消息"] -->|"查路由并发送"| BM
    C["Consumer<br/>消费消息"] -->|"查路由并拉取"| BM
    P -.->|"注册 / 心跳"| NS
    C -.->|"注册 / 心跳"| NS
    BM -.->|"注册"| NS
```
...
> **存储结构数据流**：消息顺序写入 CommitLog，ConsumeQueue 记录索引，IndexFile 支持关键字查询。

```mermaid
flowchart LR
    P["Producer 发送"] -->|"顺序写"| CL[("CommitLog<br/>完整消息体<br/>物理偏移量")]
    CL -->|"生成索引"| CQ[("ConsumeQueue<br/>per Topic/Queue<br/>offset + 大小")]
    CL -->|"建索引"| IF[("IndexFile<br/>关键字 → 物理偏移")]
    CQ -->|"定位"| C["Consumer 拉取"]
    IF -->|"关键字查询"| C
```
...
> **三种顺序消息实现**：分区顺序、全局顺序、集群顺序在队列划分与吞吐之间的权衡。

```mermaid
flowchart TB
    A["分区顺序消息<br/>同批有序消息发同一 Queue<br/>单线程消费"] -->|"简单但并发低"| B["全局顺序消息<br/>指定唯一 MessageQueue<br/>单线程消费全部"]
    A -->|"按属性分散到多 Queue"| C["集群顺序消息<br/>自定义 MessageQueueSelector<br/>多队列并行 + 队内有序"]
    B -.->|"吞吐最低"| X["牺牲并行能力"]
    C -.->|"吞吐较高"| Y["兼顾顺序与扩展"]
```
