...
随着硬件指令集的发展，我们可以使用基于冲突检测的乐观并发策略: 先进行操作，如果没有其它线程争用共享数据，那操作就成功了，否则采取补偿措施(**自旋** 不断地重试，直到成功为止)。这种乐观的并发策略的许多实现都不需要将线程阻塞，因此这种同步操作称为非阻塞同步。

**乐观锁**需要操作和冲突检测这两个步骤具备原子性，这里就不能再使用互斥同步来保证了，只能靠硬件来完成。硬件支持的原子性操作最典型的是: **比较并交换**(Compare-and-Swap，CAS)。CAS 指令需要有 3 个操作数，分别是内存地址 V、旧的预期值 A 和新值 B。当执行操作时，只有当 V 的值等于 A，才将 V 的值更新为 B。
...
2. 如果不相等，则用CAS尝试修改当前ID，如果CAS修改成功，那还是能获取到锁，执行同步代码，如果CAS失败，说明有竞争，此时升级为轻量级锁。
3. 在轻量级锁状态下，当前线程会在栈帧下创建LockRecord，LockRecord会把MarkWord的信息拷贝进去，且有个Owner指针指向加锁的对象。
4. 线程执行到同步代码时，则用CAS试图将MarkWord的指向到线程栈帧的LockRecord，假设CAS修改成功，则获取得到轻量级锁。
5. 假设修改失败，则自旋（重试），自旋一定次数之后则升级为重量级锁。
...
2. 如果**更新成功**了就获取到这个对象的轻量级锁
3. 如果**更新失败**了首先会检查对象的Mark Word是否指向当前的线程，如果指向当前的线程，说明该线程已经获取这个这个对象的锁了，继续执行同步块代码。
4. 如果不指向当前线程，表示有其他线程竞争锁，当前线程便尝试自旋获取锁。如果在这过程中获取到了，那就执行同步块代码。
   如果**自旋**一定次数还没竞争到锁，就将锁升级为重量级锁，当前线程阻塞。
5. 如果持有线程释放锁失败（CAS替换Mark Word，因为有其他线程在争夺锁），那么将释放锁并唤醒等待的[线程](https://blog.csdn.net/weixin_42737868/article/details/105730958)。
...
| 锁    | 区别           | 优点                                          | 缺点                                    | 使用场景              |
| ---- | ------------ | ------------------------------------------- | ------------------------------------- | ----------------- |
| 偏向锁  | 只有一个线程进入临界区。 | 加锁和解锁不需要CAS操作，没有额外的性能消耗，和执行非同步方法相比仅存在纳秒级的差距 | 如果线程间存在锁竞争，会带来额外的锁撤销的消耗               | 适用于只有一个线程访问同步快的场景 |
| 轻量级锁 | 多个线程交替进入临界区  | 竞争的线程不会阻塞，提高了响应速度                           | 如线程成始终得不到锁竞争的线程，使用自旋会消耗CPU性能          | 追求响应时间，同步快执行速度非常快 |
| 重量级锁 | 多个线程同时进入临界区  | 线程竞争不适用自旋，不会消耗CPU                           | 线程阻塞，响应时间缓慢，在多线程下，频繁的获取释放锁，会带来巨大的性能消耗 | 追求吞吐量，同步快执行速度较长   |
