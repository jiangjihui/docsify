...
> **TCP 三次握手**：客户端与服务器通过 SYN、SYN/ACK、ACK 三段报文协商初始序列号并建立连接。
...
sequenceDiagram
    ...
    Note over C,S: 第一次握手：请求建立连接
    S->>C: SYN + ACK, seq=server_isn, ack=client_isn+1
    Note over C,S: 第二次握手：确认并同步
    C->>S: ACK, ack=server_isn+1
    Note over C,S: 第三次握手：确认服务器 SYN
```
...
> **滑动窗口机制**：发送窗口取接收窗口 rwnd 与拥塞窗口 cwnd 的较小值，收到 ACK 后窗口向前滑动。

```mermaid
flowchart LR
    R["接收方<br/>通告接收窗口 rwnd"] -.->|"ACK 携带"| S
    subgraph S["发送方"]
        W["发送窗口 = min(rwnd, cwnd)"]
    end
    S -->|"连续发送多段<br/>无需逐段等待 ACK"| R
    R -->|"返回 ACK"| S
    W -.->|"收到 ACK 后<br/>窗口向前滑动"| W
```
...
> **拥塞控制四阶段**：慢开始指数增长，到阈值后转入拥塞避免线性增长，丢包触发快重传与快恢复。

```mermaid
flowchart TD
    START["连接建立 / 超时<br/>cwnd = 1 MSS"] --> SS["慢开始<br/>每 RTT cwnd 翻倍"]
    SS -->|"cwnd ≥ ssthresh"| CA["拥塞避免<br/>每 RTT cwnd +1 MSS"]
    CA -->|"连续 3 个重复 ACK"| FR["快重传<br/>立即重传丢失段"]
    FR --> REC["快恢复<br/>ssthresh = cwnd/2<br/>cwnd = ssthresh，每 ACK +1"]
    REC --> CA
```
