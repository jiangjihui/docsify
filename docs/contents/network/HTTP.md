...
> **HTTP 状态码分类**：五大类状态码按首位数字划分，分别表示信息、成功、重定向、客户端错误、服务器错误。

```mermaid
flowchart LR
    C1["1XX 信息性<br/>接收的请求正在处理"]
    C2["2XX 成功<br/>请求正常处理完毕"]
    C3["3XX 重定向<br/>需附加操作完成请求"]
    C4["4XX 客户端错误<br/>服务器无法处理请求"]
    C5["5XX 服务器错误<br/>服务器处理请求出错"]
```
...
> **HTTPS 混合加密握手**：通过非对称加密交换预主密钥，再生成对称会话密钥进行加密通信。
...
sequenceDiagram
    ...
    C->>S: 客户端问候（算法列表 + 随机数）
    S->>C: 服务器问候（选定算法 + 证书 + 随机数）
    C->>C: 验证服务器证书
    C->>S: 密钥交换（非对称加密发送预主密钥）
    C->>S: 双方生成会话密钥
    Note over C,S: 使用会话密钥进行对称加密通信
```
