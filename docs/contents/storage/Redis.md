...
> **部署模式演进**：从单机到主从、哨兵，再到分片集群，逐步解决高可用与扩展性问题。

```mermaid
flowchart TB
    S1["单机模式<br/>单实例 · 单点故障"] --> S2["主从模式<br/>master 写 / slave 读 · 单向复制"]
    S2 --> S3["哨兵模式 Sentinel<br/>监控 + 自动故障切换"]
    S3 --> S4["集群模式 Cluster<br/>数据分片(hash slot) · 分布式 · 自动故障转移"]
```
...
> **缓存三大问题对比**：穿透、击穿、雪崩的成因与典型应对手段各不相同。

```mermaid
flowchart TB
    subgraph P1["缓存穿透"]
        D1["缓存和数据库都没有的数据"] --> R1["请求每次打到存储层<br/>校验 / 空值缓存 / 布隆过滤器"]
    end
    subgraph P2["缓存击穿"]
        D2["缓存无但数据库有（热点过期）"] --> R2["并发同时查库 · 压力瞬间增大<br/>热点不过期 / 限流熔断 / 互斥锁"]
    end
    subgraph P3["缓存雪崩"]
        D3["大量 key 同时过期"] --> R3["海量查询压垮数据库<br/>过期时间随机 / 热点不过期 / 分布式均匀"]
    end
```
...
> **RDB 与 AOF 持久化对比**：RDB 是某一时刻快照，AOF 是写后日志；4.0 起支持混合使用。

```mermaid
flowchart TB
    subgraph RDB["RDB（快照）"]
        R1["fork 子进程做快照"] --> R2["某一时刻数据快照<br/>体积小 · 恢复快"]
        R3["缺点：实时性差 · 频繁 fork 重"]
    end
    subgraph AOF["AOF（写后日志）"]
        A1["先执行命令，再记日志"] --> A2["记录每条写命令<br/>可读高 · 默认每秒刷盘"]
        A3["缺点：文件大 · 重写开销"]
    end
    RDB --> M["4.0 混合：快照 + 两次快照间 AOF"]
    AOF --> M
```
