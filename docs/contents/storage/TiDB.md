...
> **TiDB 整体架构**：计算层（TiDB Server）、存储层（TiKV）、调度层（PD）职责分离。

```mermaid
flowchart TB
    C["客户端 / 应用"] --> SQL["TiDB Server<br/>SQL 层 · 解析执行计划"]
    SQL --> PD["PD<br/>元数据 · 调度 · 服务发现"]
    SQL --> KV["TiKV Server<br/>分布式 KV 存储 · 事务"]
    PD -.->|"调度与路由"| KV
```
...
> **TiKV 数据分片与多副本**：表数据自动切分为 Region 并默认维护 3 副本，分散在多节点。

```mermaid
flowchart TB
    T["表数据"] -->|"自动分片"| R1["Region 1"]
    T -->|"自动分片"| R2["Region 2"]
    T -->|"自动分片"| R3["Region N"]
    R1 -->|"多副本（默认 3 副本）"| RP1[("副本 a / b / c<br/>分散多节点")]
    R2 -->|"多副本（默认 3 副本）"| RP2[("副本 a / b / c")]
    R3 -->|"多副本（默认 3 副本）"| RP3[("副本 a / b / c")]
```
