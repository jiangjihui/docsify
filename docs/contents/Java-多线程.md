...
- **任务执行的稳定性要求高**
  ...
  如果任务类型是长时间运行的任务或者对执行时间有较高的稳定性要求，保持固定数量的线程（即核心线程数和最大线程数相等）可以确保任务的执行不会因为线程的动态创建和销毁而受到影响。

- **任务量相对稳定**
  
  当任务的到达速率相对稳定且可预测，并且能够大致估算出系统可以同时处理的任务数量时，将核心线程数和最大线程数设置为相同的值可以使线程池刚好满足任务处理的需求，不会出现因线程数过多或过少而导致的性能问题。

##### 核心线程数和最大线程数不相等

- **任务负载波动较大**
  
  当系统任务负载具有明显的波峰和波谷特征时，例如电商系统在促销活动期间流量剧增，平时流量较为平稳。将核心线程数设置为满足日常任务处理的数量，而最大线程数设置为可以应对峰值负载的数量，这样可以在低负载时节省资源，在高负载时保证系统的响应能力。

- **资源有限且任务类型多样**
  
  当系统需要处理多种类型的任务，其中一些任务是短时间内频繁提交的小任务，而另一些是长时间运行的大任务时，可以根据不同任务的特点和资源需求来设置线程池参数。核心线程可以优先处理对响应时间要求较高的小任务，当有长时间运行的大任务提交时，根据需要创建非核心线程来处理，以避免大任务长时间占用核心线程，影响系统对小任务的响应速度。
