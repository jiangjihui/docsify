...
> 先从快速导览表格跳到自己关心的章节；不确定名词时查术语表。

### 快速导览（给人和给 AI 的回车键）

| 你想解决什么问题 | 跳到哪一节 | 那节告诉你什么 |
|---|---|---|
| 怎么让 Agent 有"业务总控"而不是只会拆任务 | [Agent 编排](#agent-编排) | Commander 五步流程、专属技能委派 |
| 怎么让 LLM 查业务数据而不是编数据 | [能力层](#能力层) | 技能三层架构、防幻编四抓手 |
| 怎么让"子代理算出的结果原样到用户面前" | [能力层](#能力层) | 透传中间件 + 假流式模型 |
| 怎么让"某段规则只在特定时刻才生效" | [能力层](#能力层) | 门控中间件按场景 strip prompt 段 |
| model 开启思考后多轮报错怎么办 | [能力层](#能力层) | 子类覆写请求封装 |
| 流式渲染大 HTML 表格怎么不卡 | [子代理与前端](#子代理与前端) | rehypeRaw + 分段结构化渲染 |
| 全局：哪些边界坑必踩 | 各节的"关键坑"行 | 每节都标了红色关键坑 |
| 我要新写一个业务技能，有哪些该一口气做对 | [关键启发与实战](#关键启发与实战) | 技能开发流程 + 大数据量处理 + 迭代演进五阶段 |
| 技能加载数百上千条数据会不会把上下文塞爆 | [关键启发与实战](#关键启发与实战) | 超阈值写 CSV + 下游脚本分析，不进上下文 |
| 部署上线最容易在哪几个地方翻车 | [部署与附录](#部署与附录) | 部署架构 + 排障清单（现象→改法→为什么）|
| 我要复刻一个能连真实数据的技能，怎么对接接口 | [部署与附录](#部署与附录) | 请求/响应契约模板 + 复刻顺序 |
| 技能 stdout 到底该输出什么形状，怎么让 LLM 判读 | [部署与附录](#部署与附录) | 逐技能 JSON Schema + 通用信号约定 |
...
| 图 | 名称 | 所在区块 | 用途 |
|---|---|---|---|
| 图 1 | 整体架构图 | [背景](#背景) | 浏览器到后端四服务的总体分流 |
| 图 2 | Commander 五步流程 | [Agent 编排](#agent-编排) | 意图识别→澄清→分解委派→汇总→风控 |
| 图 3 | 中间件链示意 | [能力层](#能力层) | 严格顺序 + 各中间件触发条件 |
| 图 4 | 技能 结构/委派时序 | [能力层](#能力层) | 技能目录三层 + 专属委派调用流 |
| 图 5 | 部署拓扑图 | [部署与附录](#部署与附录) | 反代层/运行时层/存储层拓扑 |
| 图 6 | 技能迭代生命周期 | [关键启发与实战](#关键启发与实战) | 五个演进阶段的状态迁移 |
...
> 通用框架给你的是一副骨架，落到垂直领域要补齐四类增强：Agent 编排、业务数据、确定性输出、私有化基础设施。
...
> 把通用编排器改造成领域总控 Commander：意图识别、澄清、分解委派、汇总、风控的五步业务流，以及专属技能委派机制。
...
> 给 Agent 的领域能力三件套：技能层（数据怎么查）、中间件层（规则怎么在运行时强制）、模型补丁（厂商思考字段怎么保留）。
...
契约要点：
- HTML 表 + 类名标记 → 前端对该技能内容走专门渲染分支（见「HTML 表格的流式渲染管线」）。
...
- 前端据此从 `additional_kwargs.reasoning_content` 提取并渲染思考折叠面板（见「思考时长实时展示」）。
...
> 领域能力收敛到专用子代理（配置模板 + 主从分工），前端做产品化体验：思考时长、模型选择、子任务卡片、HTML 表格流式渲染。
...
> 私有化落地的三块底盘：双层线程存储、Nacos 配置/服务发现/热改技能、Langfuse/LangSmith 链路追踪。
...
> 十张普适卡片 + 技能从 0 到工程化的正确开发姿势与迭代演进规律。
...
6. **一次到位做大数据量分流**（见「大数据量返回的分流处理」），别等"数据多了再补"。
...
Python 环境里若有 `sitecustomize.py`（部署级兼容补丁，见「部署期"加代码解决"的两个经典补丁」），解释器会在**每个 Python 子进程启动时自动加载**，往 stdout 打启动横幅；技能脚本自己也常打印"开始/结束"标记行。这些噪音会让 `json.loads(整段 stdout)` 必失败。三条防御线（可叠加）：
...
> 生产部署架构与排障清单，加上把方案射影到你的领域的四份附录（场景表 / 清单速查 / 系统提示 / 接口与输出契约）。
...
- **双层存储**：LangGraph checkpoint 存全量状态 + Gateway 的轻量线程索引表（见「双层线程存储（PostgreSQL 线程索引）」），两者各自落库。
...
- 副作用：此补丁会向每个子进程 stdout 打启动横幅 → 见「沙盒 stdout 噪音净化」。
...
| 私有化部署排障 | 通用能力 | 见「私有化部署排障清单」（环境变量覆盖 + 显式诊断） |
...
| 技能迭代收敛 | 演进五阶段 | 见「技能迭代的五个演进阶段」（约束收敛 + 工程化收口）|
| 部署排障 | 部署/镜像/启动 | 见「私有化部署排障清单」（缓存层/连接池/SSE 缓冲）|
...
- 想找某类问题的解法：先翻「快速导览」表格定位章节，再读对应小节。
- 不确定某名词：查「术语表」。
- 要把方案迁移到自己的项目：查「附录 A 场景射影表」。
- 直接落地：参考「附录 B 可复用清单速查」。
- 写新技能 / 排查大数据量返回：读「技能开发与迭代实战」（含 stdout 契约、大数据量分流、演进五阶段）。
- 上线 / 排查部署故障：读「部署上线与排障实战」（含部署架构、事件循环补丁、排障清单）。
...
> 两个**防幻编要点**（对齐「防 LLM 幻编的四个抓手」）：`businessCode`/`fieldGroupCode` **必须**来自"目录/详情"接口返回，禁止 LLM 凭记忆填；`bizId` **必须**经"名称→ID 映射"接口精确匹配得到，禁止编造。
...
所有意图落地到 LLM/前端的最终结果，统一走"技能脚本的 stdout JSON"（对齐「技能脚本的 stdout 契约」），而不是裸透传接口响应：
...
4. 用「新技能的推荐开发步骤」的 6 步写 SKILL.md，把 stdout 契约的"何时停/何时重试"规则写进去。
5. 配 Sub-Agent 委派 + 白名单（见「领域子代理配置模板」），把能力隔离给专用子代理。
...
- 这些 schema 的**语义**（为什么这样设计、LLM 怎么用）见「技能脚本的 stdout 契约」；**大数据分流**见「大数据量返回的分流处理」；**沙盒噪音净化**见「沙盒 stdout 噪音净化」。
