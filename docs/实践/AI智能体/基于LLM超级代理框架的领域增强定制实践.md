...
- [1. 快速导览（给人和给 AI 的回车键）](#1-快速导览给人和给-ai-的回车键)
- [2. 术语表](#2-术语表)
- [3. 背景：从"通用框架"到"领域智能体"](#3-背景从通用框架到领域智能体)
- [4. 后端 Agent 层：Commander 总控模式](#4-后端-agent-层commander-总控模式)
- [5. 技能层：SKILL.md + scripts + frameworks 三层架构](#5-技能层skillmd--scripts--frameworks-三层架构)
- [6. 中间件层：用 runtime 机制强制 prompt 规则](#6-中间件层用-runtime-机制强制-prompt-规则)
- [7. 模型补丁：多轮对话中保留"思考内容"](#7-模型补丁多轮对话中保留思考内容)
- [8. 子代理（Sub-Agent）定制](#8-子代理sub-agent定制)
- [9. 前端产品化定制](#9-前端产品化定制)
- [10. 基础设施与私有化增强](#10-基础设施与私有化增强)
- [11. 关键启发与落地清单](#11-关键启发与落地清单)
- [12. 技能开发与迭代实战](#12-技能开发与迭代实战)
- [13. 部署上线与排障实战](#13-部署上线与排障实战)
- [附录 A：场景射影表（把本文映射到你的领域）](#附录-a场景射影表把本文映射到你的领域)
- [附录 B：可复用清单速查](#附录-b可复用清单速查)
- [附录 C：给你的 AI / RAG 的系统提示语（推荐）](#附录-c给你的-ai--rag-的系统提示语推荐)
- [附录 D：领域接口契约模板（给复刻者的对接参照）](#附录-d领域接口契约模板给复刻者的对接参照)
- [附录 E：技能输出契约（JSON Schema 规范表）](#附录-e技能输出契约json-schema-规范表)
...
|---|---|---|
| 怎么让 Agent 有"业务总控"而不是只会拆任务 | [第 4 节](#4-后端-agent-层commander-总控模式) | Commander 五步流程、专属技能委派 |
| 怎么让 LLM 查业务数据而不是编数据 | [第 5 节](#5-技能层skillmd--scripts--frameworks-三层架构) | 技能三层架构、防幻编四抓手 |
| 怎么让"子代理算出的结果原样到用户面前" | [第 6.2 节](#62-子代理结果原样透传中间件) | 透传中间件 + 假流式模型 |
| 怎么让"某段规则只在特定时刻才生效" | [第 6.3 节](#63-阈值规则按上下文门控注入中间件) | 门控中间件按场景 strip prompt 段 |
| model 开启思考后多轮报错怎么办 | [第 7 节](#7-模型补丁多轮对话中保留思考内容) | 子类覆写请求封装 |
| 流式渲染大 HTML 表格怎么不卡 | [第 9.4 节](#94-html-表格的流式渲染管线) | rehypeRaw + 分段结构化渲染 |
| 全局：哪些边界坑必踩 | [第 6-9 节的"关键坑"行](#6-中间件层用-runtime-机制强制-prompt-规则) | 每节都标了红色关键坑 |
| 我要新写一个业务技能，有哪些该一口气做对 | [第 12 节](#12-技能开发与迭代实战) | 技能开发流程 + 大数据量处理 + 迭代演进五阶段 |
| 技能加载数百上千条数据会不会把上下文塞爆 | [第 12.3 节](#123-大数据量返回的分流处理) | 超阈值写 CSV + 下游脚本分析，不进上下文 |
| 部署上线最容易在哪几个地方翻车 | [第 13 节](#13-部署上线与排障实战) | 部署架构 + 10 个排障清单（现象→改法→为什么）|
| 我要复刻一个能连真实数据的技能，怎么对接接口 | [附录 D](#附录-d领域接口契约模板给复刻者的对接参照) | 请求/响应契约模板 + 复刻顺序 |
| 技能 stdout 到底该输出什么形状，怎么让 LLM 判读 | [附录 E](#附录-e技能输出契约json-schema-规范表) | 逐技能 JSON Schema + 通用信号约定 |

**图例索引**（全文 Mermaid 图一览，均就近放置于对应章节）：

| 图 | 名称 | 所在章节 | 用途 |
|---|---|---|---|
| 图 1 | 整体架构图 | [3. 背景](#3-背景从通用框架到领域智能体) | 浏览器到后端四服务的总体分流 |
| 图 2 | Commander 五步流程 | [4.2](#42-commander-五步流程可复用的业务流转模型) | 意图识别→澄清→分解委派→汇总→风控 |
| 图 3 | 中间件链示意 | [6.5](#65-中间件链的严格顺序) | 严格顺序 + 各中间件触发条件 |
| 图 4 | 技能 结构/委派时序 | [5.1](#51-领域技能统一范式) / [4.3](#43-专属技能委派delegate-only-skill机制) | 技能目录三层 + 专属委派调用流 |
| 图 5 | 部署拓扑图 | [13.1](#131-生产部署架构参考) | 反代层/运行时层/存储层拓扑 |
| 图 6 | 技能迭代生命周期 | [12.5](#125-技能迭代的五个演进阶段可预见性) | 五个演进阶段的状态迁移 |
...
| **Delegate-Only Skill（专属技能）** | 打标记"仅某子代理可执行"的技能，Main 不允许自读，必须委派 | 见 4.3 |
...
| **frameworks/** | 按"维度×对象数"组合的**输出骨架模板**（markdown） | 见 5.2 |
| **rehype / Streamdown** | 前端 Markdown→HTML 渲染管线插件体系 | 见 9.4 |
| **Checkpoint（LangGraph）** | LangGraph 的对话状态持久化机制 | 与 10.1 双层存储配合 |
| **reasoning_content** | 部分模型思考内容字段，需补丁保留它在多轮上下文 | 见第 7 节 |
...
### 4.2 Commander 五步流程（可复用的业务流转模型）

```mermaid
flowchart TD
    A["Step1 意图识别"] --> A1{"业务请求?<br/>市场政策?<br/>不当/无关?"}
    A1 -->|"不当请求 / 无关对话"| R["直接拒绝 / 不入域"]
    A1 -->|"市场政策分析"| M["基模 + 联网搜索<br/>（不调政策知识库）"]
    A1 -->|"业务请求"| A2["Step2 引导澄清<br/>至多1轮 · 至多5维度 · 选项式"]
    ...
    A3 --> C{"复杂任务?"}
    C -->|"否（简单）"| D["直接回答"]
    C -->|"是"| E["拆解并委派 Sub-Agent<br/>（含 Delegate-Only 专属委派）"]

    E --> S["回收 Sub-Agent 输出"]
    S --> A4["Step4 汇总输出"]
    S -. "领域分析结果原样透传（passthrough）" .-> A4

    A4 --> A5["Step5 过程风控<br/>越界/缺数据/异常 → 提醒人工接管"]
...
### 4.3 专属技能委派（Delegate-Only Skill）机制
...
## 5. 技能层：SKILL.md + scripts + frameworks 三层架构
...
### 6.2 子代理结果"原样透传"中间件（PassthroughMiddleware）
...
### 6.3 阈值规则"按上下文门控注入"中间件（ThresholdGateMiddleware）
...
### 6.4 并发子代理截断中间件（SubagentLimitMiddleware）
...
### 12.2 技能脚本的 stdout 契约（LLM 的"判读语言"）
