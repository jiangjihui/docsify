...
> **故障日期**: 2024-12-25、2024-12-30（同根因两子案例）
> **涉及服务**: 数据服务B
> **故障类型**: Pod重启
...
## 根因
主根因是 pod 内存 limit 与 JVM 堆配置不一致（pod 等于或不足 JVM 堆 + 系统开销），容器内存先于 JVM 堆达到上限，cgroup 触发 OOMKiller 杀死进程。两个子案例触发与次生效应不同：2024-12-30 由大请求（3250 个 node_ids、响应体约 7.1MB）直接触发内存申请失败；2024-12-25 在 pod 重启后引发连接池雪崩——获取数据库连接的代码未加锁，并发请求瞬时获取过多连接，超过数据库 512 的连接限制。
