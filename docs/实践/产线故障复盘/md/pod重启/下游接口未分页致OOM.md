...
> **故障日期**: 2026-04-01、2026-04-14、2026-07-02（同根因三子案例）
> **涉及服务**: 业务服务A
> **故障类型**: Pod重启
...
当时未找到真实原因，曾推测为"月初大数据量查询 → GC 停顿 → liveness 超时"。事后结合 2026-04-14、2026-07-02 两起同类事故确认，真实根因相同：下游指标接口未实现真分页，导报表等大批量查询时全量数据堆积致内存压力接近 limit，服务变慢致 liveness 超时。
...
下游指标接口"有分页参数"但未实现真分页：接口对外暴露 `current`/`size` 等分页参数，但内部并未真正按分页截取数据，每次请求都返回全量结果。业务服务A 在请求大批量系统ID（2000+ 个）的日级/小时级指标数据时（2026-04-01 月初导报表、2026-04-14 日级综合效率、2026-07-02 小时级数据），客户端按"分页"循环调用，把每次返回的全量数据都 add 到内存中的 list，数据堆积无法释放，最终触发 OOM 或内存压力致 liveness 超时。

三个子案例触发场景不同（2026-04-01 为系统数据订正任务/导报表、2026-04-14 为算法服务指标接口、2026-07-02 为数据服务B 的指标数据接口），但根因相同：下游接口未实现真分页，客户端误把全量响应当分页结果累加。其中 2026-04-01 当时误判为"GC 停顿致 liveness 超时"，实际为下游未分页大响应体堆积致内存压力。
...
- 增加 pod 内存上限：短期可将 pod 内存 limit 与 JVM 堆适当上调（2026-04-01 曾将 3GB→4GB 并放宽 liveness `timeoutSeconds: 3→5`），作为缓冲，但根因仍需下游真分页修复。
...
- 现象链相似时优先怀疑同一根因：2026-04-01/04-14/07-02 三起事故均表现为"内存接近 limit + 下游大响应体告警"，真实根因均为下游未分页，避免重复误判。
...
# grafana 内存 used 接近 limit（2026-04-01: 3GB；2026-04-14: max 6GB；2026-07-02: 接近 6GB）
# livenessProbe httpGet /actuator:9099 timeout 3s period 5s failure 3（2026-04-01 曾放宽至 5s）
# 下游告警：响应体大于 10MB（2026-04-01）
# Skywalking 最长 Duration 约 440991ms（业务服务A 的 QueryDataTask，2026-04-14）
# 单条响应约 1.4MB × 254 条 ≈ 355MB（纯文本，Map 化后更大，2026-04-14）
# Feign 日志：current=8148, size=1000（分页页码异常递增，表明下游未真分页，2026-07-02）
