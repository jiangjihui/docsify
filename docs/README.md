## 简介

> 个人知识库，系统性梳理技术知识点，形成知识框架。

大部分内容摘录自各大网站和博客以及AI，部分来源于个人经验积累。尊重原创版权，引用处均标注来源，建议直接阅读原文以获取更详实的内容。

## 知识导航

| 基础 | 语言 | 框架 | 存储 |
| --- | --- | --- | --- |
| [计算机基础](contents/公共知识.md) | [Java](contents/java/Java基础.md) | [Spring 生态](contents/spring/Spring.md) | [数据存储](contents/storage/存储基础.md) |
| [AI](contents/ai/ai-agent.md) | [前端](contents/frontend/单页应用.md) | [微服务](contents/microservice/微服务基础.md) | [中间件](contents/middleware/消息队列.md) |

| 运维 | 网络 | 开发 | 大数据 |
| --- | --- | --- | --- |
| [基础设施](contents/infra/Nginx.md) | [网络](contents/network/网络协议.md) | [开发实践](contents/dev/版本控制.md) | [大数据](contents/bigdata/大数据基础.md) |

## 专题

- [设计模式](设计模式/) — 24种GoF设计模式及对比
- [架构](架构/) — DDD等架构实践
- [场景方案](场景方案/) — 缓存与数据库一致性等方案
- [代码分析](代码分析/) — Java/SQL代码分析
- [AI智能体实践](实践/AI智能体/基于LLM超级代理框架的领域增强定制实践.md) — 通用LLM超级代理框架的领域化改造方案

## 文档搭建

> 本文档基于 [docsify](https://github.com/docsifyjs/docsify/) 生成，所有 Markdown 转换在浏览器端执行。

[开始阅读](contents/公共知识.md)
